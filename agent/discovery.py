...
def run_discovery(seed, market='IL', model_name=None) -> dict:
    ...
    data = {'search_queries': payload.get('search_queries') if isinstance(payload.get('search_queries'), list) else [], 'sources': payload.get('sources') if isinstance(payload.get('sources'), list) else [], 'candidate_variants': extracted, 'conflicts': payload.get('conflicts') if isinstance(payload.get('conflicts'), list) else [], 'unresolved': bool(payload.get('unresolved', False)), 'unresolved_reason': payload.get('unresolved_reason'), 'field_evidence': payload.get('field_evidence', {})}
    ...
    return {'ok': ok, 'data': data if ok else None, 'error': res.get('error'), 'gemini_metadata': {
        ...
        'discovery_raw_text_debug_available': bool(res.get('raw_text')), 'discovery_parsed_top_level_keys': sorted(payload.keys()) if isinstance(payload, dict) else [],
