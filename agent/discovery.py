...
def run_discovery(seed, market='IL') -> dict:
    ...
    if not isinstance(res, dict):
        return {
            'ok': False,
            'data': None,
            'error': f'Gemini client returned non-dict: {type(res).__name__}',
            'gemini_metadata': {
                'ok': False,
                'provider': 'gemini',
                'model': None,
                ...
                'error': 'non-dict gemini response',
                'raw_text': None,
            },
        }
    ...
    data = {
        ...
        'candidate_variants': payload.get('candidate_variants') if isinstance(payload.get('candidate_variants'), list) else [],
    ...
    return {
        ...
        'gemini_metadata': {
            ...
            'model': res.get('model'),
            'grounding_requested': bool(res.get('grounding_requested', True)),
            'request_attempted': bool(res.get('request_attempted', True)),
            'error': res.get('error'),
