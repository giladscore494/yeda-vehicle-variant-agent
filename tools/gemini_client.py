...
class GeminiClient:
    ...
    def grounded_generate_json(self, prompt, schema_hint=None, strong=False, model_override=None):
        ...
        if not self.has_api_key():
            base["error"] = "GEMINI_API_KEY missing"
            return base
        if genai is None or types is None:
            base["error"] = f"google-genai import failed: {IMPORT_ERROR}"
            return base
        if self.client is None:
            base["error"] = "Gemini client not initialized"
            return base

        base["request_attempted"] = True
        try:
            config = types.GenerateContentConfig(
            ...
            raw_text = getattr(response, "text", "") or ""
            parsed_json, parsed_text, parse_error = self._parse_or_repair(model, raw_text)
            base.update({
            ...
            base["error"] = f"Gemini grounding/search call failed: {exc}"
            return base
import json
import os
