...
def test_model_mode_fast(monkeypatch):
    monkeypatch.setattr('agent.runner.run_discovery', lambda seed, market='IL', model_name=None: {'ok': True, 'data': {'sources':[1,2], 'search_queries':['q'], 'candidate_variants':[1], 'conflicts':[]}, 'gemini_metadata': {'request_attempted': True, 'grounding_requested': True}})
...
def test_model_mode_auto_escalates_when_low_sources(monkeypatch):
    calls=[]
    def fake(seed, market='IL', model_name=None):
